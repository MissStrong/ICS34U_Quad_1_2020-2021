...
The `setBounds()` function is used to make a component show up in a specific location on the frame. The first two parameters are the x and y coortinates of the top-left corner, and the last two parameters are the width (horizontal distance) and height (vertical distance) of the component.
